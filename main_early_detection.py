...
class MemeIntelligenceBot(
    ...
    def __init__(self):
        ...
        self.early_detection = EarlyDetectionEngine(
            self._send_early_alert
        )

    async def _send_early_alert(
        self,
        channel_id: int,
        message: str,
    ):
    ...
    async def setup_hook(self):
        ...
        await self.early_detection.initialize()
        await self.tree.sync()
        self.early_detection.start()
...
# =========================================================
# Early Detection Commands
# =========================================================

@bot.tree.command(
    name="early_start",
    description="بدء مراقبة العملات الجديدة على Solana",
)
async def early_start(
    interaction: discord.Interaction,
):
    await early_detection_start(
        interaction
    )
...
@bot.tree.command(
    name="early_stop",
    description="إيقاف تنبيهات Early Detection",
)
async def early_stop(
    ...
    await bot.early_detection.clear_channel()
    await bot.early_detection.stop()

    await interaction.followup.send(
        "🛑 **تم إيقاف تنبيهات Early Detection.**\n\n"
        "يمكن إعادة التشغيل باستخدام `/early_start`."
    )


@bot.tree.command(
    name="early_status",
    description="عرض حالة Early Detection",
)
async def early_status(
    interaction: discord.Interaction,
):
    status = await bot.early_detection.status()
    ...
    message = (
        ...
        f"🟢 **الحالة:** `{running}`\n"
        f"📡 **قناة التنبيهات:** {channel_text}\n"
        f"⏱️ **الفاصل:** `{status.get('interval_seconds', 0)} ثانية`\n"
        f"🗂️ **Tokens المتتبعة:** `{status.get('tracked_tokens', 0)}`\n"
        f"🚨 **التنبيهات المرسلة:** `{status.get('alerts_sent', 0)}`\n"
    )
...
@bot.tree.command(
    name="early_scan",
    description="تنفيذ فحص Early Detection الآن",
)
async def early_scan(
    interaction: discord.Interaction,
):
    await interaction.response.defer()

    await bot.early_detection.set_channel(
        interaction.channel_id
    )

    result = await bot.early_detection.scan_once()

    await interaction.followup.send(
        "━━━━━━━━━━━━━━━━━━\n"
        "🔎 **Early Detection Scan**\n\n"
        f"👁️ **العناصر المفحوصة:** "
        f"`{result.get('seen', 0)}`\n"
        f"🎯 **المرشحون بعد الفلترة:** "
        f"`{result.get('candidates', 0)}`\n"
        f"🚨 **التنبيهات الجديدة:** "
        f"`{result.get('alerts', 0)}`\n\n"
        "⚠️ الفحص تحليلي فقط ولا ينفذ أي عملية شراء."
    )


async def early_detection_start(
    interaction: discord.Interaction,
):
    await interaction.response.defer()
    ...
    bot.early_detection.start()

    await interaction.followup.send(
        "🟢 **تم تشغيل Early Detection.**\n\n"
        "سيتم مراقبة العملات الجديدة على Solana "
        "وإرسال التنبيهات التحليلية إلى هذه القناة.\n\n"
        "⚠️ التنبيهات لا تمثل توصية شراء."
    )
...
async def token_command(
    ...
    try:
        ...
        #
        # IMPORTANT:
        # Market Intelligence is calculated directly from the
        # already validated DEX Screener pair and sent as a
        # separate Discord message.
        #
        # This prevents it from disappearing inside the large
        # combined report and also removes dependency on the
        # optional "market" field returned by analysis_engine.
        # =================================================

        market = None

        try:
            market = analyze_market_intelligence(pair)
        except Exception as market_error:
            print(
                "Market Intelligence calculation error:",
                repr(market_error),
            )

            # Fallback to the analysis engine result if available.
        ...
        if market_message:
            ...
            print("MARKET_MESSAGE_SENT")

        # Start a fresh message for the remaining report.
        message = ""
